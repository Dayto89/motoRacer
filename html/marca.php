...
if (!$conexion) {
    // En lugar de morir, podemos registrar el error y mostrarlo elegantemente.
...
// --- Agregar categoría (con patrón PRG) ---
if ($_POST && isset($_POST['guardar'])) {
    ...
    if ($resultado) {
        ...
        $_SESSION['status'] = [
            'type' => 'error',
            'message' => "La Marca no fue agregada.<br><small>Error: $error</small>"
        ];
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['eliminar'])) {
    ...
    // Responder solo con JSON indicando éxito o fracaso
    header('Content-Type: application/json');
...
// --- Obtener todos los datos para la tabla inicial ---
...
<body>
    ...
    <nav class="barra-navegacion">
        <div class="ubica"> Productos/Marca </div>
        <div class="userContainer">
            <div class="userInfo">
                <?php
                    // Ya tenemos una conexión, no es necesario crear otra.
    ...
    <div id="categorias" class="form-section">
        ...
        <div class="container">
            <div class="actions">
                <button id="btnAbrirModal" class="btn-nueva-categoria"><i class='bx bx-plus bx-tada icon'></i>Nueva Marca</button>
                <input type="text" id="searchRealtime" name="valor" placeholder="Ingrese la marca a buscar">
            </div>
...
<script>
    ...
    async function validarNombre(nombre) {
        try {
            ...
            const res = await fetch('marca.php', { // Se especifica el archivo
                method: 'POST',
